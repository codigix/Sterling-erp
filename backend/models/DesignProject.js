...
class DesignProject {
  ...
  static async findAll(filters = {}) {
    ...
    if (filters.search) {
      conditions.push(
        "(project_name LIKE ? OR project_code LIKE ? OR client_name LIKE ?)"
      );
  ...
  static async findById(id) {
    const [rows] = await pool.execute(
      "SELECT * FROM design_projects WHERE id = ?",
      [id]
    );
  ...
  static async create(data) {
    ...
    const [result] = await pool.execute(
      ...
      [
        ...
        data.steelSections && data.steelSections.length > 0
          ? JSON.stringify(data.steelSections)
          : null,
        data.plates && data.plates.length > 0
          ? JSON.stringify(data.plates)
          : null,
        data.fasteners && data.fasteners.length > 0
          ? JSON.stringify(data.fasteners)
          : null,
        data.components && data.components.length > 0
          ? JSON.stringify(data.components)
          : null,
        data.electrical && data.electrical.length > 0
        ...
        data.referenceDocuments && data.referenceDocuments.length > 0
          ? JSON.stringify(data.referenceDocuments)
          : null,
        data.createdBy || null,
      ],
    );
  ...
  static async update(id, data) {
    ...
    if (data.designStatus !== undefined)
      updates.design_status = data.designStatus;
    if (data.designEngineerName !== undefined)
      updates.design_engineer_name = data.designEngineerName;
    if (data.systemLength !== undefined)
      updates.system_length = data.systemLength;
    if (data.systemWidth !== undefined) updates.system_width = data.systemWidth;
    if (data.systemHeight !== undefined)
      updates.system_height = data.systemHeight;
    if (data.loadCapacity !== undefined)
      updates.load_capacity = data.loadCapacity;
    if (data.operatingEnvironment !== undefined)
      updates.operating_environment = data.operatingEnvironment;
    if (data.materialGrade !== undefined)
      updates.material_grade = data.materialGrade;
    if (data.surfaceFinish !== undefined)
      updates.surface_finish = data.surfaceFinish;
    if (data.steelSections !== undefined)
      updates.steel_sections =
        data.steelSections && data.steelSections.length > 0
          ? JSON.stringify(data.steelSections)
          : null;
    ...
    if (data.consumables !== undefined)
      updates.consumables =
        data.consumables && data.consumables.length > 0
          ? JSON.stringify(data.consumables)
          : null;
    if (data.designSpecifications !== undefined)
      updates.design_specifications = data.designSpecifications;
    if (data.manufacturingInstructions !== undefined)
      updates.manufacturing_instructions = data.manufacturingInstructions;
    if (data.qualitySafety !== undefined)
      updates.quality_safety = data.qualitySafety;
    if (data.additionalNotes !== undefined)
      updates.additional_notes = data.additionalNotes;
    if (data.referenceDocuments !== undefined)
      updates.reference_documents =
        data.referenceDocuments && data.referenceDocuments.length > 0
          ? JSON.stringify(data.referenceDocuments)
          : null;
    ...
    await pool.execute(
      `UPDATE design_projects SET ${setClause} WHERE id = ?`,
      values,
    );
  }
  ...
  static async updateStatus(id, status) {
    await pool.execute(
      "UPDATE design_projects SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
      [status, id]
    );
  }
