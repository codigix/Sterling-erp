...
exports.createWorkflowBasedTasks = async (req, res) => {
  ...
  try {
    ...
    if (!rootCardId || !workflowSteps || workflowSteps.length === 0) {
      return res.status(400).json({ message: 'Root card ID and workflow steps are required' });
    }
    ...
    for (const step of workflowSteps) {
      const [result] = await connection.execute(
        `INSERT INTO design_workflow_steps 
         (root_card_id, step_name, estimated_hours, priority_level, description, status, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
          rootCardId,
          step.stepName,
...
exports.getAllDesignsWithDetails = async (req, res) => {
  try {
    ...
    try {
      const [designs] = await connection.execute(
        'SELECT * FROM root_cards ORDER BY created_date DESC'
      );
            ...
            for (const phaseName of Object.keys(selectedPhases)) {
              ...
              const [stageResult] = await connection.execute(
                `INSERT INTO production_stages 
                 (production_plan_id, stage_name, stage_type, status, created_date)
                 VALUES (?, ?, ?, ?, NOW())`,
