...
exports.internalCreateWorkflowTasks = async (rootCardId, userId, connection) => {
  ...
  // This is crucial because employee_tasks has a foreign key to employees(id)
  ...
  if (employees.length === 0) {
    // If the current user is not in the employees table (e.g., they are an Admin from the 'users' table)
    // We need to find a suitable employee to assign these tasks to, or at least avoid the FK error.
    // First, try to find an employee with the same login_id/username if available
    ...
    if (userRecords.length > 0) {
      ...
      if (matchingEmployees.length > 0) {
        ...
        // Fallback: Find the first active Design Engineer to assign tasks to
        const [designEngineers] = await connection.execute(
          `SELECT e.id FROM employees e 
           JOIN roles r ON e.role_id = r.id 
           WHERE r.name = 'Design Engineer' OR r.name = 'design_engineer' 
           LIMIT 1`
        );
        
        if (designEngineers.length > 0) {
          effectiveEmployeeId = designEngineers[0].id;
        } else {
          // Absolute fallback: Find any active employee
          ...
          if (anyEmployee.length > 0) {
            ...
            throw new Error('No valid employee found to assign workflow tasks to. Please ensure at least one employee exists in the system.');
          }
        }
      }
    } else {
      // If we can't even find a user record, just try to find ANY employee
      const [anyEmployee] = await connection.execute(
        'SELECT id FROM employees LIMIT 1'
      );
      if (anyEmployee.length > 0) {
        ...
        throw new Error('User not found in employees table and no alternative employee available.');
      }
    }
  }
  ...
  // Get Design Engineer role ID - prioritize the user's current role if it matches
  let roleId = null;
  const [userRole] = await connection.execute(
    "SELECT r.id FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = ? AND (r.name = 'Design Engineer' OR r.name = 'design_engineer')",
    [userId]
  );
  ...
  if (userRole.length > 0) {
    ...
    const [roles] = await connection.execute(
      "SELECT id FROM roles WHERE name = 'Design Engineer' OR name = 'design_engineer' LIMIT 1"
    );
    roleId = roles.length > 0 ? roles[0].id : null;
  }

  // Get root card details including sales_order_id - support both root_card_id and sales_order_id
  ...
  if (rootCards.length === 0) {
    // If no root card found, check if it's a valid sales order
    ...
    // Auto-create root card if it doesn't exist
    const [result] = await connection.execute(
      ...
      [
        ...
        so.id,
        so.po_number || null,
        so.project_name || 'Design Project',
        'planning',
        'medium',
        so.due_date || null,
        userId,
        JSON.stringify({ auto_created: true, from_workflow: true }),
        '[]'
      ]
    );
    
    // Now we have a root card
    const newRootCardId = result.insertId;
    const effectiveRootCardId = newRootCardId;
    const baseSalesOrderId = so.id;
    
    // Re-assign for the rest of the function
    await runWorkflowCreation(effectiveRootCardId, baseSalesOrderId);
  } else {
    const rootCard = rootCards[0];
    const effectiveRootCardId = rootCard.id;
    const baseSalesOrderId = rootCard.sales_order_id;
    await runWorkflowCreation(effectiveRootCardId, baseSalesOrderId);
  }

  async function runWorkflowCreation(effectiveRootCardId, baseSalesOrderId) {
    // Get all active workflow steps ordered by step_order
    const [workflowSteps] = await connection.execute(
      'SELECT id, step_name, description, priority, step_order, task_template_title, task_template_description FROM design_workflow_steps WHERE is_active = TRUE ORDER BY step_order ASC'
    );

    if (workflowSteps.length === 0) {
      console.warn('No active workflow steps defined');
      return;
    }

    // Create both department_tasks and employee_tasks for each workflow step
    for (const step of workflowSteps) {
      ...
      // 1. Create Department Task (if it doesn't exist)
      if (roleId) {
        const [existingDept] = await connection.execute(
          'SELECT id FROM department_tasks WHERE root_card_id = ? AND task_title = ? AND role_id = ?',
          [effectiveRootCardId, taskTitle, roleId]
        );
        ...
        if (existingDept.length === 0) {
          const [result] = await connection.execute(
            ...
            [
              ...
              userId,
              baseSalesOrderId,
              JSON.stringify({ step_order: step.step_order, auto_generated: true, workflow_step: true })
            ]
          );
          ...
          // If task already exists, still add it to the list of returned tasks 
          // so the user sees the total count of workflow tasks
          createdTasks.push({
            id: existingDept[0].id,
            stepName: taskTitle,
            priority: step.priority || 'medium',
            status: existingDept[0].status || 'pending',
            type: 'department_task',
            existing: true
          });
        }
...
exports.createWorkflowBasedTasks = async (req, res) => {
  ...
  try {
    ...
    const createdTasks = await exports.internalCreateWorkflowTasks(rootCardId, userId, connection);
